...
int main(void)
{
    ...
    int status = 0, count = 0, flag = 0;
    while (1) 
    {
        status = 0;
        while((status < 3) && (LL_GPIO_IsInputPinSet(GPIOA, LL_GPIO_PIN_0)) ) 
        {
            delay();
            status++;
        }
